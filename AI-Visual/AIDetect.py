...
capturing = False
capture_buffer=[]
MODEL_PATH= "gru_bit_model_enhanced.pt"
...
def clean_and_merge_blocks(raw_blocks,min_length=1):
...
def finalize_capture():
    ...
    if len(blocks) < 2:
        ...
        return
    print(blocks)
    threshold = math.ceil(blocks[0]['length']/8.0)+1
...
def mouse_callback(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        clicked_hsv=hsv[y,x]
        print(f"Pixel cliaue en HSV : {clicked_hsv}")

        wide_tol =np.array([20,80,80])
        lower_tmp = np.clip(clicked_hsv - wide_tol, 0,255)
        upper_tmp = np.clip(clicked_hsv + wide_tol, 0,255)
        mask = cv2.inRange(hsv,lower_tmp, upper_tmp)

        num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(mask,connectivity=8)
        label_clicked = labels[y,x]
        if label_clicked == 0 :
            print("Aucun composant detecte")
            return
        
        component_mask = (labels == label_clicked).astype(np.uint8)*255

        contours, _ =cv2.findContours(component_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if not contours:
            print("pas de contous detectes")
            return
        
        hull=cv2.convexHull(contours[0])
        mask_convex =np.zeros_like(component_mask)
        cv2.drawContours(mask_convex, [hull], -1, 255, -1)

        mean_hsv = cv2.mean(hsv,mask=mask_convex)[:3]
        hsv_val=np.array(mean_hsv,dtype=np.uint8)
        lower_bound =np.clip(hsv_val - hsv_tolerance, 0, 255)
        upper_bound = np.clip(hsv_val + hsv_tolerance, 0 ,255)


        calibrated = True
        print(f"Calibration mise à jour : HSV = {hsv_val} ± {hsv_tolerance}")
...
def main():
    ...
    picam2 = Picamera2()
    picam2.preview_configuration.main.size = (640, 480) #(160, 120)
